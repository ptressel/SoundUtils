...
public class CompositeWave implements Wave {
  ...
  public CompositeWave( double[] frequency, int[] amplitude,
           double[] initialPhase, double sampleRate, int sampleBytes,
    ...
    if ( sampleBytes < 1 || sampleBytes > 2 ) {
      throw new IllegalArgumentException(
  ...
  /**
   * <p>Set up a generator for a composite wave.  Parameters are wrapped
   * in a CompositeWaveParams (for wave characteristics) and an
   * AudioFormatParams (for audio format information).
   *
   * <p>Each component of the composite wave is a pure tone, specified by
   * its frequency, amplitude, and phase at any point in time.
   *
   ...
   * constructor will throw an IllegalArgumentException.
   *
   * @param waveParams the frequency, amplitude, and initial phase for each
   * wave
   *
   * @param formatParams the audio format information
   *
   * @throws IllegalArgumentException if an unsupported audio format is
   * specified, or if the numbers of frequencies, amplitudes, and initial
   * phases (if supplied) are not the same
   */
  public CompositeWave( CompositeWaveParams waveParams,
           AudioFormatParams formatParams )
      throws IllegalArgumentException {

    // Unpack the args and use the other constructor.
    this( waveParams.getFrequency(), waveParams.getAmplitude(),
          waveParams.getInitialPhase(), formatParams.sampleRate,
          formatParams.sampleBytes, formatParams.signed,
          formatParams.bigEndian );
  }
