...
public class SoundUtils {
   ...
   * @param sampleBytes sample width in bytes
   * @param bigEndian true if big endian
   * @param signed true if signed
   * @return the reassembled sample value
   */
  public static int extractSample( byte[] buffer, int byteOffset, int sampleBytes,
    ...
    if ( sampleBytes == 1 ) {
      ...
      throw new IllegalArgumentException(
        "SoundUtils.extractSample: sampleBytes must be 1 or 2, got "
        + sampleBytes );
    }
   ...
   *
   * @throws ArrayIndexOutOfBoundsException if byteOffset + sampleBytes extends
  ...
  public static void insertSample( double sampleValue, byte[] buffer,
                                   int byteOffset, int sampleBytes,
                                   boolean signed, boolean bigEndian )
   ...
   * byte array according to the given audio format.  Clip to fit the sample
   * size if necessary.
   *
   ...
   * format.
   *
   * <p>Supported audio formats are mono, 1 or 2 bytes per sample, signed, PCM.
   *
   * @param frequency frequency of the signal to be generated
   * @param amplitude amplitude of the signal to be generated
   *
   ...
   * @param signed audio format: true if signed (unsigned not yet supported)
  ...
  public static byte[] generateCycleFormatted(
          double frequency, int amplitude,
          double sampleRate, int sampleBytes, boolean signed,
          boolean bigEndian, int channels ) {

    // Check for unsupported audio formats.
    if ( !signed || sampleBytes < 1 || sampleBytes > 2 || channels != 1 ) {
    ...
    // Make a buffer for the formatted samples.  For each sample, we need
    // sampleBytes bytes.
    byte[] cycle = new byte[ sampleBytes * rawCycle.length ];

    // Get the amplitude limits for the various sample sizes.  For 2s
    // complement, max is ( 2^#bits / 2 ) -1 , and min is - ( 2^#bits / 2 ).
    int bits = Byte.SIZE * sampleBytes;
    ...
    for ( int i = 0; i < rawCycle.length; i++ ) {
      ...
      insertSample( value, cycle, i*sampleBytes, sampleBytes, signed,
                    bigEndian );
      /*debug*/
      /*
  ...
  private static boolean oneTest( byte[] input, int[] results, int sampleBytes,
                                 boolean signed, boolean bigEndian ) {

    boolean allOk = true;  // Assume the best.

    // Loop over the results.  Index into input is result index * sampleBytes.
    for ( int r = 0, i = 0; r < results.length; r++, i += sampleBytes ) {
      // Call extractSample.
      int convSays = extractSample( input, i, sampleBytes, signed, bigEndian );
      // Compare with expected result.
      if ( convSays != results[r] ) {
        System.out.println( "Input: " + input[i]
          + (sampleBytes == 2 ? (", " + input[i+1]) : "")
